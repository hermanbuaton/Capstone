...
class Rake_model extends CI_Model {
    ...
    /**
     * Custom sort for insert to db
     */
    private function custom_sort($arr)
    {
        $out = array();
	...
	/**
     *  Load latest version of stop words list
	 *  
     *  original implementation is retrieving them from .txt file
     *  see controller/Apple/read_file
	 */
	public function load_stopwords()
	{
		$ver = $this->latestVer_get();
        $list = $this->stopwords_get($ver);
        return $list;
	}
    ...
    /**
     *  Load id of latest ver of word list
	 */
	private function latestVer_get()
	{
		$row = $this->db
                    ->select('MAX(ver_id) AS ver')
                    ->from('rake_stop_ver')
                    ->get()->row();
    ...
    /**
     *  Load stop words of $ver from db
     */
	private function stopwords_get($ver)
	{
		// retrieve from db
        $words = $this->db
                    ->select('word')
                    ->from('rake_stop_words')
                    ->where('ver_id',$ver)
                    ->get()->result_array();
     ...
     */
    public function version_post($time)
    {
        $this->db->insert('rake_stop_ver', array('ver_date'=>$time));
    ...
    /**
     *  Put words of new version
     */
    public function words_post($data)
    {
        $this->db->insert_batch('rake_stop_words', $data);
        return;
    }
